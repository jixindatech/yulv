...
var CollationNames = map[string]int{
	"big5_chinese_ci":          1,
	"latin2_czech_cs":          2,
	"dec8_swedish_ci":          3,
	"cp850_general_ci":         4,
...
func ValidateCharset(cset, collation string) error {
	_, ok := CharsetIds[cset]
	if !ok {
		return fmt.Errorf("%s", "invalid charset")
	}

	_, ok = CollationNames[collation]
	if !ok {
		return fmt.Errorf("%s", "invalid collation")
	}

	if strings.HasPrefix(collation, cset+"_") != true {
		return fmt.Errorf("%s", "invalid charset and collation")
	}
